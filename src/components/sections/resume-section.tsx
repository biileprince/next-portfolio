...
import type { Education, Experience, Achievement, SkillsByCategory } from "@/types";
...
export function ResumeSection({ education, experience, achievements, skills }: ResumeSectionProps) {
  const [activeTab, setActiveTab] = useState<TabId>("education");

  return (
    <section id="resume" className="w-full py-20 border-b border-surface-700/30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <SectionTitle title="Professional Journey" subtitle="Education, Experience & Technical Expertise" />

        {/* Tabs */}
        <div className="flex flex-wrap gap-3 mb-10 border-b border-surface-700/30 pb-4">
          {tabs.map((tab) => (
            <button
              key={tab.id}
                ...
                {achievements.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-xs text-brand-400 font-medium mb-1.5">{item.year}</span>
                      <h4 className="text-base font-semibold text-surface-100 mb-2">{item.title}</h4>
                      <p className="text-surface-400 text-sm flex-grow">{item.description}</p>
                    </motion.div>
                  );
                })}
              </motion.div>
    ...
    <div className="glass rounded-xl p-6 hover:border-brand-500/20 transition-all duration-300">
      <div className="flex items-start gap-4 mb-4">
        ...
        <div className="min-w-0">
          <h4 className="text-base font-semibold text-surface-100">{item.title}</h4>
          <p className="text-sm text-surface-400">{item.subtitle}</p>
          <div className="flex items-center gap-3 mt-1.5">
            <span className="text-xs text-brand-400 font-medium">{item.period}</span>
      ...
      <ul className="space-y-2 ml-2">
        {details.map((detail, idx) => (
          <li key={idx} className="flex items-start gap-2 text-sm text-surface-400">
